...
export const agentApi = {
  async getAgents(params?: QueryAgentParams): Promise<AgentListResponse> {
    const response = await apiClient.get<BaseResponse<AgentListResponse>>('/agents', {
      params
    })
  ...
  async getFeaturedAgents(): Promise<Record<AgentCategory, Agent[]>> {
    const response = await apiClient.get<BaseResponse<Record<AgentCategory, Agent[]>>>('/agents/featured')
    return response.data.data
  },

  async getPopularAgents(limit: number = 10): Promise<Agent[]> {
    const response = await apiClient.get<BaseResponse<Agent[]>>('/agents/popular', {
      params: { limit }
    })
    return response.data.data
  },
  ...
  async getCategoryStats(): Promise<CategoryStats> {
    const response = await apiClient.get<BaseResponse<CategoryStats>>('/agents/categories/stats')
    return response.data.data
  },

  async getAllTags(): Promise<TagsResponse> {
    const response = await apiClient.get<BaseResponse<TagsResponse>>('/agents/tags')
    return response.data.data
  },
  ...
  async updateAgent(id: number, data: UpdateAgentDto): Promise<Agent> {
    const response = await apiClient.patch<BaseResponse<Agent>>(`/agents/${id}`, data)
    return response.data.data
  },
